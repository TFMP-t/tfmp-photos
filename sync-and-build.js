...
.school-card.warn::before{background:linear-gradient(180deg,#8a4a12,#7d1f2c)}
.school-card.neutral::before{background:linear-gradient(180deg,#3a4f5c,#6d8794)}
.school-card:hover{box-shadow:var(--shadow-hover);transform:translateY(-3px);border-color:rgba(13,132,156,.35)}
...
.school-icon.ok{background:linear-gradient(135deg,#04543f,#0d849c)}
.school-icon.warn{background:linear-gradient(135deg,#8a4a12,#7d1f2c);box-shadow:0 3px 10px rgba(125,31,44,.30)}
.school-icon.neutral{background:linear-gradient(135deg,#3a4f5c,#6d8794);box-shadow:0 3px 10px rgba(58,79,92,.25)}
...
.stat-chip{display:flex;align-items:center;gap:5px;font-size:.71rem;font-weight:600;padding:4px 10px;border-radius:20px;background:var(--surface-2);border:1px solid var(--line-soft);color:var(--muted);transition:transform .15s var(--ease)}
...
.lightbox.open img{animation:lbIn .18s var(--ease)}
  ...
  <div class="head-inner">
    ...
    <div class="filters">
      ...
      <div class="field"><select id="sortSelect">
        ...
        <option value="needs_attention_first">الأكثر احتياجًا للمتابعة أولًا</option>
...
<main>
  ...
  <div class="grid" id="listRoot"><div class="no-results">جارٍ تحميل بيانات المدارس…</div></div>
</main>
...
// تاريخ آخر إنجاز، كل التقييمات العامة المسجَّلة لها، وأسوأ تصنيف من بينها
// (يُستخدم في تحديد المدارس التي تحتاج متابعة فعليًا)
function schoolStats(school){
  const orders = Object.entries(school.workOrders || {});
  let photoCount = 0, lastDate = null;
  const ratings = new Set();
  let worstTone = null; // null = لا يوجد تقييم مسجَّل بعد
  const toneRank = { bad: 2, neutral: 1, good: 0 };
  orders.forEach(([wo, data])=>{
    ...
    if(data.overall_rating){
      ratings.add(data.overall_rating);
      const tone = ratingTone(data.overall_rating);
      if(worstTone === null || toneRank[tone] > toneRank[worstTone]) worstTone = tone;
    }
  });
  return { woCount: orders.length, photoCount, lastDate, ratings, worstTone };
}

Promise.all([
  fetch('schools-data.json').then(r=>{
    if(!r.ok) throw new Error('تعذّر الوصول إلى ملف بيانات المدارس (HTTP ' + r.status + ')');
    return r.json();
  }),
  ...
  console.error('فشل تحميل بيانات المدارس:', err);
  document.getElementById('kpis').innerHTML = '';
  document.getElementById('resultCount').textContent = '';
  document.getElementById('listRoot').innerHTML =
    '<div class="no-results">تعذّر تحميل بيانات المدارس. يُرجى تحديث الصفحة، فإذا استمرت المشكلة تحقَّق من وجود ملف schools-data.json داخل مجلد docs.<br><span class="mono" style="font-size:.7rem;color:var(--faint)">' +
    String(err.message || err).replace(/</g,'&lt;') + '</span></div>';
});
...
function sortEntries(entries){
  ...
  const toneRank = { bad: 2, neutral: 1, good: 0 };
  const rankOf = tone => tone === null ? -1 : toneRank[tone];
  ...
  withStats.sort((a, b) => {
    ...
    switch(mode){
      ...
      case 'needs_attention_first':
        return rankOf(statsB.worstTone) - rankOf(statsA.worstTone);
      case 'photos_desc':
...
function render(){
  ...
  root.innerHTML = entries.map(([code, school])=>{
    ...
    const latestRating = [...stats.ratings].pop();
    let ratingChip;
    if(latestRating){
    ...
    // أيقونة الحالة: مؤشر بصري سريع مبني على أسوأ تقييم عام حقيقي للمدرسة،
    // بدلًا من عرض حرفين من اسم المدرسة (مكرَّر مع الاسم المكتوب بجانبه أصلًا)
    const statusIcon = stats.worstTone === 'bad' ? '⚠' : (stats.worstTone === 'good' ? '✓' : '🏫');
    const statusClass = stats.worstTone === 'bad' ? 'warn' : (stats.worstTone === 'good' ? 'ok' : 'neutral');
    return (
      '<div class="school-card ' + statusClass + '" data-code="' + code.replace(/"/g,'&quot;') + '">' +
        '<div class="sc-top">' +
